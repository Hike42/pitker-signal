...
function Partners({ lang, full = false }: { lang: Language; full?: boolean }) {
  ...
  return <div className={`partner-grid ${full ? 'partner-grid-full' : ''}`}>{PARTNERS.map(p => <article className="partner-card" key={p.id} id={`partner-${p.id}`}>
    <Link href={href(`/people#partner-${p.id}`,lang)} aria-label={fr ? `Découvrir ${p.name}` : `Meet ${p.name}`}><div className="partner-portrait"><Image src={p.imageOff} width={500} height={625} loading="lazy" alt={p.name} sizes="(max-width: 760px) 100vw, 40vw"/></div></Link>
...
function Practices({lang,heading=true}:{lang:Language;heading?:boolean}) {
 return <section className={`${isAtelier?'a-practices':isSignal?'s-practices':'h-expertise'} wrap`} id="expertises">
  ...
  {isAtelier ? sections.map((s,i)=><div className="a-practice" key={s.slug}><Link className="a-practice-row" href={href(`/practices/${s.slug}`,lang)}><span className="label">0{i+1}</span><h3>{title(s,lang)}</h3><p>{description(s,lang)}</p><A/></Link></div>) : isSignal ? <div className="s-practice-grid">{sections.map((s,i)=><Link className="s-practice" href={href(`/practices/${s.slug}`,lang)} key={s.slug}><div className="label"><span>0{i+1} / {title(s,lang)}</span><A/></div><div className={`shape ${['shape-circles','shape-grid','shape-orbit','shape-stairs'][i]}`} aria-hidden="true"/><div><h3>{title(s,lang)}.</h3><p>{description(s,lang)}</p></div></Link>)}</div> : <><div className="h-sector-list">{sections.slice(0,3).map((s,i)=><Link className="h-sector" href={href(`/practices/${s.slug}`,lang)} key={s.slug}><Image src={s.image} width={600} height={800} loading="lazy" alt="" sizes="(max-width: 760px) 100vw, 33vw"/><div className="label"><span>0{i+1} / {title(s,lang)}</span><A/></div><div><h3>{title(s,lang)}</h3><p>{description(s,lang)}</p></div></Link>)}</div><Link className="h-ceo" href={href('/practices/ceo-search',lang)}><h3>CEO Search</h3><p>{description(sections[3],lang)}</p><A/></Link></>}
 </section>;
}
...
function Home({lang}:{lang:Language}) {
 const t=translations[lang];
 return <>{!isHorizon&&<Header lang={lang} path="/" dark={isSignal}/>}<main id="main">{isHorizon?<div className="h-cinema"><Image className="h-backdrop" src="/design/horizon.png" width={1672} height={941} alt="" preload sizes="100vw"/><Header lang={lang} path="/" dark/><HomeHero lang={lang}/></div>:<HomeHero lang={lang}/>}
...
function InteriorHero({lang,eyebrow,heading,lead,image}:{lang:Language;eyebrow:string;heading:string;lead?:string;image?:string}) {
 return <section className={`interior-head ${image?'has-image':''}`}><div className="wrap interior-head-grid"><div><Link className="label breadcrumb" href={href('/',lang)}>{lang==='fr'?'Accueil':'Home'} / {eyebrow}</Link><h1>{heading}</h1>{lead&&<p className="interior-lead">{lead}</p>}</div>{image?<figure className="interior-image"><Image src={image} width={800} height={900} alt="" preload sizes="(max-width: 760px) 100vw, 50vw"/></figure>:<div className="interior-star" aria-hidden="true"><Star size={213}/></div>}</div></section>;
}
...
function PracticeDetail({lang,slug}:{lang:Language;slug:string}) {
 ...
 return <><InteriorHero lang={lang} eyebrow={practiceTitle(lang)} heading={title(item,lang)} lead={description(item,lang)} image={item.image}/><section className="reading-section wrap"><aside><p className="label">{practiceTitle(lang)}</p><div className="practice-nav">{sections.map(s=><Link key={s.slug} href={href(`/practices/${s.slug}`,lang)} aria-current={s.slug===slug?'page':undefined}>{title(s,lang)} <A/></Link>)}</div></aside><div className="prose"><BodyParagraphs items={t.intro}/></div></section><section className="strengths-section"><div className="wrap"><div className="section-top"><h2>{t.strengths.title}</h2></div><div className="strengths-grid">{t.strengths.points.map((p,i)=><article key={p.title}><span className="label">0{i+1}</span><h3>{p.title}</h3><p>{p.description}</p></article>)}</div></div></section><section className="practice-contact wrap"><div><p className="label">{translations[lang].practices.contact.title}</p><h2>{partner.name}</h2><p>{partner.role[lang]} · {partner.major[lang]}</p><Link className="text-link" href={href(`/people#partner-${partner.id}`,lang)}>{fr?'Découvrir son parcours':'Discover their background'} <A/></Link><a className="text-link" href={`mailto:${partner.email}`}>{fr?'Entrer en relation':'Get in touch'} <A/></a></div><Image src={partner.imageOff} alt={partner.name} width={400} height={440} loading="lazy" sizes="(max-width: 760px) 100vw, 50vw"/></section></>;
}
function Contact({lang}:{lang:Language}) {
 const t=translations[lang].contact;
 return <><InteriorHero lang={lang} eyebrow={translations[lang].navigation.contact} heading={t.hero.title} lead={t.hero.description}/><section className="contact-section wrap"><div className="contact-note"><h2>{t.hero.scrollCTA}</h2></div><div className="contact-list">{PARTNERS.map(p=><article key={p.id}><Image src={p.imageSmall} alt={p.name} width={88} height={88} sizes="88px"/><div><h3>{p.name}</h3><p>{p.major[lang]}</p><a href={`mailto:${p.email}`}>{p.email} <A/></a></div></article>)}</div></section><section className="office-section"><div className="wrap office-grid"><div><p className="label">{t.location.title}</p><h2>143 Boulevard<br/>Haussmann</h2><p>75008 Paris, France</p><a className="text-link" href="https://www.google.com/maps/search/?api=1&query=143+Boulevard+Haussmann+75008+Paris" target="_blank" rel="noopener noreferrer">{lang==='fr'?'Ouvrir dans Google Maps':'Open in Google Maps'} <A/></a></div><ContactMap lang={lang}/></div></section></>;
}
